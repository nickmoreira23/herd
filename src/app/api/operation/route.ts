...
export async function POST(request: Request) {
  try {
    ...
    const { name, description, icon, color, items } = result.data;
...
export async function PATCH(request: Request) {
  try {
    const result = await parseAndValidate(
      request,
      z.object({ categoryIds: z.array(z.string()) })
    );
    if ("error" in result) return result.error;

    await Promise.all(
      result.data.categoryIds.map((id, index) =>
        prisma.opexCategory.update({
          where: { id },
    ...
    console.error("PATCH /api/operation error:", e);
