...
export async function GET(request: Request) {
  try {
    ...
    const subscriptions = await svc.listSubscriptions({ status, page, limit });
    return apiSuccess(subscriptions);
  } catch (e) {
    console.error("GET /api/integrations/recharge/subscriptions error:", e);
    return apiError("Failed to fetch Recharge subscriptions", 500);
  }
}
