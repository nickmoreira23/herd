...
export async function GET(request: Request) {
  try {
    ...
    const result = await svc.listContacts({ startingAfter, perPage });
    return apiSuccess(result);
  } catch (e) {
    console.error("GET /api/integrations/intercom/contacts error:", e);
    return apiError("Failed to fetch Intercom contacts", 500);
  }
}
