...
export async function GET(request: Request) {
  try {
    ...
    const result = await svc.listConversations({ startingAfter, perPage });
    return apiSuccess(result);
  } catch (e) {
    console.error("GET /api/integrations/intercom/conversations error:", e);
    return apiError("Failed to fetch Intercom conversations", 500);
  }
}
