...
export async function GET(request: Request) {
  try {
    ...
    const result = await svc.listTickets({ page, limit, status });
    return apiSuccess(result);
  } catch (e) {
    console.error("GET /api/integrations/gorgias/tickets error:", e);
    return apiError("Failed to fetch Gorgias tickets", 500);
  }
}
