...
export async function GET(request: Request) {
  try {
    ...
    const result = await svc.listCustomers({ page, limit });
    return apiSuccess(result);
  } catch (e) {
    console.error("GET /api/integrations/gorgias/customers error:", e);
    return apiError("Failed to fetch Gorgias customers", 500);
  }
}
