...
export async function POST(request: Request) {
  try {
    ...
    const parsed = await parseImportWorkbook(communityConfig, buffer);

    if (parsed.errors.length > 0 && parsed.rows.length === 0) {
      return apiError(parsed.errors.join("; "), 400);
    }

    const result = await executePartialUpdates(communityConfig, parsed.rows);
    return apiSuccess({ ...result, parseErrors: parsed.errors, detectedColumns: parsed.detectedColumns });
  } catch (e) {
    console.error("POST /api/community/import error:", e);
    return apiError("Failed to import community benefits", 500);
  }
}
