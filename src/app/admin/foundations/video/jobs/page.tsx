...
export default function VideoJobsPage() {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/foundation/video/jobs?${params}`);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          History of all video operations — transcriptions, generations, and uploads.
        </p>
      </div>
              ...
              jobs.map((job) => (
                <tr key={job.id} className="border-b last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-2 text-muted-foreground">
                    {job.videoDurationSec ? `${Math.round(job.videoDurationSec)}s` : "—"}
                  </td>
                  <td className="px-4 py-2 text-muted-foreground">
                    {job.sourceType ?? "—"}
                  </td>
