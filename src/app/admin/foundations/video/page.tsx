...
export default async function VideoDashboardPage() {
  ...
  const [totalJobs, completedJobs, failedJobs, recentJobs, transcribeCount, generateCount] =
    await Promise.all([
      prisma.videoJob.count(),
      prisma.videoJob.count({ where: { status: "completed" } }),
      prisma.videoJob.count({ where: { status: "failed" } }),
      prisma.videoJob.findMany({
        orderBy: { createdAt: "desc" },
        take: 10,
      }),
      prisma.videoJob.count({ where: { operation: "transcribe" } }),
      prisma.videoJob.count({ where: { operation: "generate" } }),
    ]);
  ...
  const stats = [
    ...
    { label: "Generations", value: generateCount },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link
            href="/admin/foundations/video/providers"
            className="rounded-md border px-3 py-1.5 text-sm font-medium hover:bg-muted"
          >
            Providers
          </Link>
          <Link
            href="/admin/foundations/video/jobs"
            className="rounded-md border px-3 py-1.5 text-sm font-medium hover:bg-muted"
          >
            Job History
          </Link>
      ...
      <div className="rounded-lg border p-5">
        ...
        {recentJobs.length === 0 ? (
          <p className="text-sm text-muted-foreground py-4 text-center">
            No video jobs yet. Jobs appear here when transcriptions or generations are run.
          </p>
        ) : (
              ...
              >
                ...
                <div className="flex items-center gap-3 text-muted-foreground">
                  {job.videoDurationSec && (
                    <span>{Math.round(job.videoDurationSec)}s</span>
                  )}
                  <span>
