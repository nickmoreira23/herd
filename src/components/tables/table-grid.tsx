...
function getColWidth(field: TableFieldRow): number {
...
function SortableRow({
  ...
  record: TableRecordRow;
  idx: number;
  fields: TableFieldRow[];
  columnWidths: Record<string, number>;
  editingCell: { recordId: string; fieldId: string } | null;
  onCellClick: (recordId: string, field: TableFieldRow) => void;
  renderCell: (
    record: TableRecordRow,
    field: TableFieldRow
  ) => React.ReactNode;
...
export function TableGrid({
  ...
  const [records, setRecords] =
    useState<TableRecordRow[]>(initialRecords);
  ...
  const refreshRecords = useCallback(async () => {
    const res = await fetch(
      `/api/tables/${table.id}/records?limit=100`
    );
  ...
  const addRecord = useCallback(async () => {
    const res = await fetch(`/api/tables/${table.id}/records`, {
      method: "POST",
  ...
  const deleteRecord = useCallback(
    async (recordId: string) => {
      const res = await fetch(
        `/api/tables/${table.id}/records/${recordId}`,
        { method: "DELETE" }
      );
      if (res.ok) {
  ...
  const handleCellClick = useCallback(
    (recordId: string, field: TableFieldRow) => {
      ...
      resizeSaveRef.current = setTimeout(async () => {
        ...
        setColumnWidths((latestWidths) => {
          ...
          if (width) {
            ...
            fetch(
              `/api/tables/${table.id}/fields/${fieldId}`,
              {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
  ...
  const handleColumnDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      try {
        const res = await fetch(
          `/api/tables/${table.id}/fields/reorder`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  const handleRowDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      try {
        await fetch(
          `/api/tables/${table.id}/records/reorder`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
