...
import { getKnowledgeFeedColumns } from "./knowledge-feed-columns";
import { KnowledgeAddFeedModal } from "./knowledge-add-feed-modal";
import { KnowledgeFeedDeleteDialog } from "./knowledge-feed-delete-dialog";
import { KnowledgeFeedEntriesDialog } from "./knowledge-feed-entries-dialog";
import { KnowledgeFeedSettingsDialog } from "./knowledge-feed-settings-dialog";
...
import type { KnowledgeRSSFeedRow, KnowledgeRSSFeedStats } from "./types";
...
interface KnowledgeFeedTableProps {
  initialFeeds: KnowledgeRSSFeedRow[];
  initialStats: KnowledgeRSSFeedStats;
}

export function KnowledgeFeedTable({
  initialFeeds,
  initialStats,
}: KnowledgeFeedTableProps) {
  const [feeds, setFeeds] = useState<KnowledgeRSSFeedRow[]>(initialFeeds);
  const [stats, setStats] = useState<KnowledgeRSSFeedStats>(initialStats);
  const [search, setSearch] = useState("");
  const [statusValue, setStatusValue] = useState("All Status");
  const [showAdd, setShowAdd] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<KnowledgeRSSFeedRow | null>(null);
  const [entriesTarget, setEntriesTarget] = useState<KnowledgeRSSFeedRow | null>(null);
  const [settingsTarget, setSettingsTarget] = useState<KnowledgeRSSFeedRow | null>(null);
  ...
  const refreshFeeds = useCallback(async () => {
    const res = await fetch("/api/knowledge/feeds");
  ...
  const handleSync = useCallback(
    async (feed: KnowledgeRSSFeedRow) => {
      toast.info("Syncing feed...");
      const res = await fetch(`/api/knowledge/feeds/${feed.id}/sync`, {
        method: "POST",
      });
      if (res.ok) {
        const json = await res.json();
  ...
  const handleToggleActive = useCallback(
    async (feed: KnowledgeRSSFeedRow) => {
      const res = await fetch(`/api/knowledge/feeds/${feed.id}`, {
        method: "PATCH",
  ...
  return (
    <>
      ...
      <KnowledgeAddFeedModal
        open={showAdd}
        onOpenChange={setShowAdd}
        onComplete={refreshFeeds}
      />

      <KnowledgeFeedDeleteDialog
        open={!!deleteTarget}
        onOpenChange={(open) => {
