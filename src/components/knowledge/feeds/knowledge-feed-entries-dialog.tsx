...
import type { KnowledgeRSSFeedRow, KnowledgeRSSEntryRow } from "./types";

interface KnowledgeFeedEntriesDialogProps {
  feed: KnowledgeRSSFeedRow | null;
  open: boolean;
...
export function KnowledgeFeedEntriesDialog({
  feed,
  open,
  onOpenChange,
  onSync,
}: KnowledgeFeedEntriesDialogProps) {
  if (!feed) return null;

  const [entries, setEntries] = useState<KnowledgeRSSEntryRow[]>([]);
  ...
  useEffect(() => {
    if (open) {
      fetch(`/api/knowledge/feeds/${feed.id}/entries`)
        .then((r) => r.json())
        .then((json) => {
    ...
    async (entryId: string) => {
      ...
      try {
        const res = await fetch(
          `/api/knowledge/feeds/${feed.id}/entries/${entryId}`
        );
