...
interface FormBuilderProps {
  initialForm: KnowledgeFormDetail;
}
...
export function FormBuilder({ initialForm }: FormBuilderProps) {
  ...
  const [form, setForm] = useState<KnowledgeFormDetail>(initialForm);
  const [sections, setSections] = useState(initialForm.sections);
  const [activeTab, setActiveTab] = useState("builder");
  const [editingField, setEditingField] =
    useState<KnowledgeFormFieldRow | null>(null);
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    if (dragType === "section") {
      ...
      await fetch(`/api/knowledge/forms/${form.id}/sections/reorder`, {
        method: "POST",
    ...
    if (dragType === "field") {
      ...
      if (crossSection) {
        await fetch(
          `/api/knowledge/forms/${form.id}/fields/${activeFieldId}`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
      ...
      if (targetFields) {
        await fetch(`/api/knowledge/forms/${form.id}/fields/reorder`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const handleAddSection = useCallback(async () => {
    ...
    await fetch(`/api/knowledge/forms/${form.id}/sections`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const handleUpdateSection = useCallback(
    async (
      ...
      await fetch(`/api/knowledge/forms/${form.id}/sections/${sectionId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleEditField = useCallback((field: KnowledgeFormFieldRow) => {
  ...
  const handleDeleteField = useCallback(
    async (fieldId: string) => {
      await fetch(`/api/knowledge/forms/${form.id}/fields/${fieldId}`, {
        method: "DELETE",
      });
      await refreshForm();
    },
  ...
  const handlePublish = useCallback(async () => {
    const res = await fetch(`/api/knowledge/forms/${form.id}/publish`, {
      method: "POST",
    });
    if (res.ok) {
  ...
  const handleClose = useCallback(async () => {
    const res = await fetch(`/api/knowledge/forms/${form.id}/close`, {
      method: "POST",
    });
    if (res.ok) {
...
function FieldDragPreview({ field }: { field: KnowledgeFormFieldRow }) {
...
function SectionDragPreview({
  ...
  section: KnowledgeFormSectionRow;
}) {
  return (
