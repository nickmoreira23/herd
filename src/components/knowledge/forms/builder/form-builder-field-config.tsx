...
import type { KnowledgeFormFieldRow } from "../types";

interface FormBuilderFieldConfigProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  field: KnowledgeFormFieldRow | null;
  formId: string;
  sectionId: string;
  onSave: () => void;
...
export function FormBuilderFieldConfig({
  ...
  async function handleSave() {
    ...
    try {
      if (isEdit && field) {
        await fetch(`/api/knowledge/forms/${formId}/fields/${field.id}`, {
          method: "PATCH",
        ...
        await fetch(`/api/knowledge/forms/${formId}/fields`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
