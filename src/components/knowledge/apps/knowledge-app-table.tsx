...
import { getKnowledgeAppColumns } from "./knowledge-app-columns";
import { KnowledgeAppConnectModal } from "./knowledge-app-connect-modal";
import { KnowledgeAppAuthModal } from "./knowledge-app-auth-modal";
import { KnowledgeAppDeleteDialog } from "./knowledge-app-delete-dialog";
import { KnowledgeAppSettingsDialog } from "./knowledge-app-settings-dialog";
...
import type { KnowledgeAppRow, KnowledgeAppStats } from "./types";
...
interface KnowledgeAppTableProps {
  initialApps: KnowledgeAppRow[];
  initialStats: KnowledgeAppStats;
}

export function KnowledgeAppTable({
  initialApps,
  initialStats,
}: KnowledgeAppTableProps) {
  const router = useRouter();
  const [apps, setApps] = useState<KnowledgeAppRow[]>(initialApps);
  const [stats, setStats] = useState<KnowledgeAppStats>(initialStats);
  ...
  const [deleteTarget, setDeleteTarget] = useState<KnowledgeAppRow | null>(null);
  const [settingsTarget, setSettingsTarget] = useState<KnowledgeAppRow | null>(null);
  const searchParams = useSearchParams();

  const refreshApps = useCallback(async () => {
    const res = await fetch("/api/knowledge/apps");
  ...
  const handleSync = useCallback(
    async (app: KnowledgeAppRow) => {
      toast.info(`Syncing ${app.name}...`);
      const res = await fetch(`/api/knowledge/apps/${app.id}/sync`, {
        method: "POST",
      });
      if (res.ok) {
        await refreshApps();
  ...
  const handleDisconnect = useCallback(
    async (app: KnowledgeAppRow) => {
      const res = await fetch(`/api/knowledge/apps/${app.id}/connect`, {
        method: "DELETE",
      });
      if (res.ok) {
        await refreshApps();
