...
import type { KnowledgeAudioRow } from "../types";
...
interface KnowledgeAudioViewerProps {
  audio: KnowledgeAudioRow | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onUpdate: () => void;
}

export function KnowledgeAudioViewer({
  audio,
  open,
  onOpenChange,
  onUpdate,
}: KnowledgeAudioViewerProps) {
  ...
  async function handleReprocess() {
    ...
    setProcessing(true);
    try {
      const res = await fetch(`/api/knowledge/audios/${audio.id}/process`, {
        method: "POST",
      });
      if (res.ok) {
