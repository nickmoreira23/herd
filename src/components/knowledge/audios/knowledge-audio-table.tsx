...
import { getKnowledgeAudioColumns } from "./knowledge-audio-columns";
import { KnowledgeAudioUploadModal } from "./knowledge-audio-upload-modal";
import { KnowledgeDeleteDialog } from "../knowledge-delete-dialog";
import { KnowledgeAudioViewer } from "./knowledge-audio-viewer";
...
interface KnowledgeAudioTableProps {
  initialAudios: KnowledgeAudioRow[];
  initialFolders: KnowledgeFolderRow[];
}

export function KnowledgeAudioTable({
  initialAudios,
  initialFolders,
}: KnowledgeAudioTableProps) {
  const [audios, setAudios] = useState<KnowledgeAudioRow[]>(initialAudios);
  ...
  const [deleteTarget, setDeleteTarget] = useState<KnowledgeAudioRow | null>(null);
  const [deleteFolderTarget, setDeleteFolderTarget] = useState<KnowledgeFolderRow | null>(null);
  const [viewerAudio, setViewerAudio] = useState<KnowledgeAudioRow | null>(null);
  ...
  const handleView = useCallback((audio: KnowledgeAudioRow) => {
    setViewerAudio(audio);
  }, []);

  const handleDownload = useCallback((audio: KnowledgeAudioRow) => {
  ...
  const handleToggleActive = useCallback(
    async (audio: KnowledgeAudioRow) => {
      const res = await fetch(`/api/knowledge/audios/${audio.id}`, {
        method: "PATCH",
  ...
  const handleDeleteAudio = useCallback(
    async (audio: KnowledgeAudioRow) => {
      const res = await fetch(`/api/knowledge/audios/${audio.id}`, { method: "DELETE" });
      if (res.ok) {
        await refreshData();
  ...
  const handleMoveToFolder = useCallback(
    async (audio: KnowledgeAudioRow, folderId: string | null) => {
      const res = await fetch(`/api/knowledge/audios/${audio.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    <>
      ...
      <KnowledgeAudioViewer
        ...
        onUpdate={async () => {
          await refreshData();
          if (viewerAudio) {
            const res = await fetch(`/api/knowledge/audios/${viewerAudio.id}`);
