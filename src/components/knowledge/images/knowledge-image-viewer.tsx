...
import type { KnowledgeImageRow } from "../types";
...
interface KnowledgeImageViewerProps {
  image: KnowledgeImageRow | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onUpdate: () => void;
}

export function KnowledgeImageViewer({
  image,
  open,
  onOpenChange,
  onUpdate,
}: KnowledgeImageViewerProps) {
  ...
  async function handleReprocess() {
    ...
    setProcessing(true);
    try {
      const res = await fetch(`/api/knowledge/images/${image.id}/process`, {
        method: "POST",
      });
      if (res.ok) {
