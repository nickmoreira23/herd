...
import { KnowledgeCrawlProgress } from "./knowledge-crawl-progress";
import type { KnowledgeLinkRow, KnowledgeLinkPageRow } from "./types";

interface KnowledgeLinkContentDialogProps {
  link: KnowledgeLinkRow | null;
  open: boolean;
...
export function KnowledgeLinkContentDialog({
  link,
  open,
  onOpenChange,
  onRescrape,
}: KnowledgeLinkContentDialogProps) {
  if (!link) return null;
...
function FullSiteContentDialog({
  ...
  const [pages, setPages] = useState<KnowledgeLinkPageRow[]>(
    link.pages || []
  );
  ...
  useEffect(() => {
    if (open && link.scrapeMode === "FULL_SITE") {
      fetch(`/api/knowledge/links/${link.id}`)
        .then((r) => r.json())
        .then((json) => {
          if (json.data?.pages) {
    ...
    async (pageId: string) => {
      ...
      try {
        const res = await fetch(
          `/api/knowledge/links/${link.id}/pages/${pageId}`
        );
