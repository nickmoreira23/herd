...
import { Loader2, RefreshCw, Users, UserPlus, MessageSquare, Hash } from "lucide-react";
import type { IntegrationOverviewProps } from "./index";

// ─── Types ───────────────────────────────────────────────────────

interface XTwitterStats {
  username: string;
  followers: number;
  following: number;
  tweetCount: number;
...
export default function XTwitterOverview({
  isConnected,
}: IntegrationOverviewProps) {
  const [stats, setStats] = useState<XTwitterStats | null>(null);
  ...
  const fetchStats = useCallback(() => {
    ...
    fetch("/api/integrations/x-twitter/stats")
      .then((r) => r.json())
      .then((json) => {
        if (json.data) setStats(json.data);
        else setError(json.error || "Failed to load X stats");
      })
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-10">
          ...
          <p className="text-sm text-muted-foreground text-center">
            Connect X to view your profile stats and tweets.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">X Overview</CardTitle>
        </CardHeader>
        <CardContent>
          {stats?.username && (
