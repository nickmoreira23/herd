...
import { Loader2, RefreshCw, CheckCircle2, Layers, Activity, User } from "lucide-react";
...
export default function ReplicateOverview({
  isConnected,
  ...
  const fetchStats = useCallback(() => {
    ...
    fetch("/api/integrations/replicate/stats")
      .then((r) => r.json())
      .then((json) => {
        if (json.data) setStats(json.data);
        else setError(json.error || "Failed to load Replicate stats");
      })
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-10">
          ...
          <p className="text-sm text-muted-foreground text-center">
            Connect Replicate to access thousands of open-source AI models.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Replicate Overview</CardTitle>
        </CardHeader>
        <CardContent>
          {stats?.username && (
