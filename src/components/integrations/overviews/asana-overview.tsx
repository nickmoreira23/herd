...
export default function AsanaOverview({
  isConnected,
}: IntegrationOverviewProps) {
  const [workspaces, setWorkspaces] = useState<AsanaWorkspace[] | null>(null);
  ...
  const fetchData = useCallback(() => {
    fetch("/api/integrations/asana/workspaces")
      .then((r) => r.json())
      .then((json) => {
    ...
    fetch("/api/tasks?source=asana&limit=0")
      .then((r) => r.json())
      .then((json) => {
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await fetch("/api/integrations/asana/import", {
        method: "POST",
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-10">
          ...
          <p className="text-sm text-muted-foreground text-center">
            Connect Asana to import your tasks and projects.
          </p>
        </CardContent>
      </Card>
    );
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-10">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Try refreshing the page or reconnecting Asana.
          </p>
        </CardContent>
      </Card>
    );
  ...
  const totalProjects = workspaces?.reduce((sum, w) => sum + (w.projects?.length ?? 0), 0) ?? 0;
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center gap-2.5 rounded-lg border px-3 py-2.5">
              <LayoutList className="h-4 w-4 text-orange-500 shrink-0" />
              <div>
                <p className="text-sm font-semibold tabular-nums">{totalProjects}</p>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-xs text-muted-foreground mb-3">
            Import all tasks from Asana into your workspace. Duplicates are automatically skipped.
          </p>
          <Button
                ...
                <div key={ws.gid}>
                  ...
                  <div className="space-y-2">
                    {ws.projects && ws.projects.length > 0 ? (
                      ws.projects.slice(0, 8).map((proj) => (
                        <div
                          key={proj.gid}
