...
import { Loader2, RefreshCw, Users, Eye, Video, Play } from "lucide-react";
...
export default function YouTubeOverview({
  ...
  const fetchStats = useCallback(() => {
    ...
    fetch("/api/integrations/youtube/stats")
      .then((r) => r.json())
      .then((json) => {
        if (json.data) setStats(json.data);
        else setError(json.error || "Failed to load YouTube stats");
      })
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-10">
          ...
          <p className="text-sm text-muted-foreground text-center">
            Connect YouTube to view your channel stats and videos.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">YouTube Overview</CardTitle>
        </CardHeader>
        <CardContent>
          {stats?.channelName && (
            <p className="text-sm font-medium mb-4">{stats.channelName}</p>
          )}
          <div className="grid grid-cols-2 gap-4">
