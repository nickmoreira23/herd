...
import { COMMUNITY_PRESETS, COMMUNITY_CATEGORIES } from "./community-presets";
import { CommunityDetailClient, type CommunityFormState } from "./community-detail-client";
...
export function CommunityCreateWizard({ allTiers }: CommunityCreateWizardProps) {
  const [step, setStep] = useState<"choose" | "form">("choose");
  const [search, setSearch] = useState("");
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [prefilledForm, setPrefilledForm] = useState<Partial<CommunityFormState> | null>(null);

  const filteredPresets = useMemo(() => {
    let results = COMMUNITY_PRESETS;
    if (selectedCategory) {
  ...
  if (step === "form") {
    return <CommunityDetailClient allTiers={allTiers} prefill={prefilledForm ?? undefined} />;
  }
    ...
    <div className="flex flex-col -m-6 min-h-[100vh]">
      ...
      <div className="flex items-center justify-between shrink-0 border-b py-3 px-4">
        <nav className="flex items-center gap-2 text-sm">
          ...
          <span className="text-foreground font-medium">New Benefit</span>
        </nav>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="text-center space-y-2">
            <h1 className="text-2xl font-bold">What kind of community benefit would you like to add?</h1>
            <p className="text-sm text-muted-foreground">
              Choose a template to get started quickly, or create a completely custom benefit.
            </p>
          </div>
          ...
          {filteredPresets.length === 0 ? (
            <div className="text-center py-12 text-sm text-muted-foreground">
              No matching benefits found. Try a different search or{" "}
              <button
                onClick={() => {
                  setPrefilledForm(null);
                  setStep("form");
