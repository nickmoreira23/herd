...
export function getImageColumns(
  ...
  return [
    ...
    },
    {
      ...
      header: () => <span className="text-xs">{t("images.columns.type")}</span>,
      cell: ({ row }) => {
    ...
    {
      id: "dimensions",
      header: () => <span className="text-xs">{t("images.columns.dimensions")}</span>,
      cell: ({ row }) => {
    ...
    {
      accessorKey: "status",
      header: () => <span className="text-xs">{t("images.columns.status")}</span>,
      cell: ({ row }) => {
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => actions.onView(image)}>
                ...
                {t("images.columns.actions.view")}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => actions.onDownload(image)}>
                <Download className="mr-2 h-3.5 w-3.5" />
                {t("images.columns.actions.download")}
              </DropdownMenuItem>
                ...
                <>
                  ...
                  <DropdownMenuSub>
                    <DropdownMenuSubTrigger>
                      ...
                      {t("images.columns.actions.move_to_folder")}
                    </DropdownMenuSubTrigger>
                    <DropdownMenuSubContent>
                      {image.folderId && (
                        <DropdownMenuItem
                        ...
                        >
                          ...
                          {t("images.columns.actions.move_to_root")}
                        </DropdownMenuItem>
                      )}
                      {rootFolders
              ...
              <DropdownMenuItem onClick={() => actions.onToggleActive(image)}>
                {image.isActive
                  ? t("images.columns.actions.deactivate")
                  : t("images.columns.actions.activate")}
              </DropdownMenuItem>
              <DropdownMenuItem
                className="text-destructive"
                onClick={() => actions.onDelete(image)}
              >
                {t("images.columns.actions.delete")}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        );
