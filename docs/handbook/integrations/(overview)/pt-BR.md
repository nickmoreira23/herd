...
A camada de Integrations conecta o HERD a sistemas externos. Stripe processa pagamentos. Google Calendar sincroniza meetings. Slack entrega notificações. Cada integration é uma ponte entre o model interno do HERD e um serviço de terceiro que já faz algo bem.

## Business

Integrations são o reconhecimento do HERD de que a empresa não opera no vácuo. Clientes já pagam via Stripe ou Recharge; calendários já vivem no Google ou Outlook; comunicação já acontece em Slack ou email. Em vez de reconstruir o que essas ferramentas fazem, o HERD se integra com elas para que usuários não precisem context-switch.
...
<!-- TODO: Como usuários (admins) descobrem, habilitam e configuram integrations no admin UI do HERD. Integration catalog, OAuth flows, configuração por vendor, status monitoring. -->
...
Integrations interagem com o resto do HERD via o graph de cross-references: um Block (como Meetings) consome uma category de integration (como Calendar Integrations) sem se comprometer com um vendor específico. A binding real do vendor acontece na camada de configuração.
