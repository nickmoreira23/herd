...
The rule that an organization must always keep at least one active owner.
...
**From [Audit Log](/admin/handbook/tools/infrastructure/audit-log):**

A `{resource}.{verb}` lowercase string identifying what happened, e.g. `location.deleted`.

### Actor

**From [Audit Log](/admin/handbook/tools/infrastructure/audit-log):**

The profile that *executed* the action. For an invitation accept, the invited person — not the inviter. Nullable; a deleted actor sets the column to NULL.
...
A tenant-scoped, immutable record that a specific actor performed a specific action on a specific resource at a specific time.
...
An audit insert whose failure is logged and swallowed, so it can never break or roll back the business action it describes.

### Billing customer

**From [Billing Customers](/admin/handbook/blocks/financial/billing-customers):**

the platform's local mirror of a customer record held by the payment provider.
...
**From [Subscriptions](/admin/handbook/blocks/financial/subscriptions):**

internal audit log row capturing a state transition.
...
**From [Charges](/admin/handbook/blocks/financial/charges):**

a single charge event in the provider.

### ChargeLineItem

**From [Charges](/admin/handbook/blocks/financial/charges):**

row that splits a composite charge across the subscriptions it covers (junction N:N).

### Coarse gating

**From [Roles & Permissions](/admin/handbook/areas/identity/roles-permissions):**

Authorization by role name (`requireOrgRole`), as opposed to fine-grained resource×action checks.
...
OAuth + read/write + inbound webhooks + state synchronization + signature validation + deduplication.

### Department-scoped role

**From [Roles & Permissions](/admin/handbook/areas/identity/roles-permissions):**

A role bound to a department via `scopeId` (`scopeType = DEPARTMENT`); defined in the model but not yet used as a gate.
...
retry of a failed charge, tracked per attempt with an outcome.
...
the customer's identifier in the provider's namespace (e.g. Recharge customer ID).
...
A resource declared in `ROLE_PERMISSIONS` with no `requireOrgRole` route to enforce against; model-only ("model — no surface").
...
Uses the shared integration infra (token store, webhook table, sync logs) without domain-specific models.
...
Raw inbound webhook payload stored before processing, enabling replay and audit.
...
Per-profile record storing OAuth tokens for integrations that support personal connections.

### Membership

**From [Roles & Permissions](/admin/handbook/areas/identity/roles-permissions):**

An `OrganizationMember` row linking a profile to an organization; carries the member's roles.
...
A role that applies to the whole organization (`scopeType = ORG`); V1 gives each member exactly one.
...
The hardcoded `ROLE_PERMISSIONS` map of role → resource×action grants; the declared access model.
...
short-lived URL to the provider's hosted billing UI.
...
The affected entity, identified by `resource_type` + a generic `resource_id` string.
...
A named bundle of access (`OWNER`, `ADMIN`, `MEMBER`, and three department-scoped variants), modeled as the `MemberRole` enum.
...
`can()` runs alongside `requireOrgRole` and logs agreement/divergence (`[can-shadow]`) without ever changing the result.

### Shallow integration

**From [Integration Model](/admin/handbook/integrations/integration-model):**

OAuth + read-only API calls. No inbound webhooks or state synchronization.
...
An RLS policy with only `tenant_isolation` (no permissive `herd_app_full_access`), rejecting cross-tenant reads and writes at the database layer.
...
an active billing relationship in the provider mirrored locally.
...
A platform-level actor (`isSuperAdmin`) that bypasses org permission checks but never the ≥1-owner invariant.
...
A category of integrations that shares domain-level business rules requiring its own models or invariants (e.g., Payment).
