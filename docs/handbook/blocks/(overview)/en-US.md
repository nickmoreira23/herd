...
The Blocks layer is ComeçaAI's foundation for **data primitives** — entities the platform tracks with stable identities and CRUD semantics. Contacts, Meetings, Deals, Documents, Images: each is a Block. Blocks are the most-consumed primitives in ComeçaAI; tools and solutions read and write to blocks constantly.

## Business

A Block is the atomic unit of *what ComeçaAI knows*. While Tools execute and Solutions deliver value, Blocks are *the data*. Every commercial interaction either creates new Block instances (a new Contact, a new Meeting, a new Deal) or modifies existing ones (Contact lifecycle changes, Deal stage progression).
...
<!-- TODO: How users (sales, marketing, support, admins) interact with blocks in ComeçaAI's admin UI. List/detail views per block type, filters and segmentation, bulk operations, related-block navigation. -->
