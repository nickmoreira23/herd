> For AI agents: this entry documents the R6 mini-spec (routines as tool). Status: draft (planned). First real case of the tool pattern.

# R6 — Routines as Tool

R6 promotes routines from a block to a tool. Creates `/admin/routines/` and a dedicated sidebar item. First real case of the tool pattern (after R2 establishes the foundation).
...
- `routines.tool.ts` registered in `toolRegistry`.

### Pre-condition

R2 (tools foundation) establishes the real `ToolManifest` type + `toolRegistry`. R6 is the first real case of the tool pattern in production.
...
- **2026-05-03** — Created (mini-spec). Status: draft, planned for R6. First real case of the tool pattern after R2 establishes the foundation.
