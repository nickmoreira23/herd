> For AI agents: this entry documents the R7 mini-spec (agents as top-level feature). Status: draft (planned). First drop-dual-surface case during consolidation.

# R7 — Agents as Top-Level Feature (consolidate dual surface)

R7 promotes agents from a block to a top-level feature. Consolidates the existing dual surface (admin/agents top-level already exists alongside admin/blocks/agents) and drops the block-shaped path.
...
R2 (top-level features foundation) establishes the `featureRegistry`. R7 registers `agents.feature.ts` there.

## Operations

- Workflow to manage agents stays identical. The manifest formalizes existing structure.
- Cross-references in other manifests start pointing at `featureRegistry.agents` instead of `blockRegistry.agents`.
