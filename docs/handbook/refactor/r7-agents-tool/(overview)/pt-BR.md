> Para agentes de IA: esta entrada documenta a mini-spec de R7 (agents as tool). Status: draft (planned). Primeiro caso de drop-dual-surface durante consolidação.

# R7 — Agents as Tool (consolidate dual surface)

R7 promove agents de block para tool. Consolida o dual surface existente (admin/agents top-level já existe paralelo a admin/blocks/agents) e dropa o caminho de bloco.
...
R2 (tools foundation) cravou `toolRegistry`. R7 registra `agents.tool.ts` lá.

## Operations

- Workflow para gerenciar agents permanece igual. Manifest formaliza estrutura existente.
- Cross-references em outros manifests passam a apontar para `toolRegistry.agents` em vez de `blockRegistry.agents`.
