...
### pg `DeprecationWarning: client.query() while already executing` — benign

You will see this in dev/server logs:
...
**It is cosmetic and benign.** The data comes out correct — `pg` queues
the second query internally and runs it after the first. `pg` is pinned
at `^8.20.0`, so there is no auto-jump to `pg@9` (which would remove the
internal auto-queue). The warning fires **once per process** (Node
de-dups deprecation warnings), which makes attributing it by eye
unreliable — use `node --trace-deprecation` to get the real call site.

**Real source — scoped reads with `include` under `withTenant`.** When a
tenant-scoped read carries an `include` (relations), the tenancy Prisma
Extension wraps the operation in a `$transaction` (to `SET app.tenant_id`),
and the Prisma 7 interpreter loads the `include`d relations as **sibling
queries that run concurrently on the same connection**. That concurrency —
*inside a single Prisma operation* — is what trips the warning. Reproduced
deterministically.

**`proxy.ts` / middleware is NOT the source — this was falsified.**
`resolveOrgByHost` calls `Organization.findUnique` on a **non-tenant-scoped**
model, so the Extension is a no-op there → each request gets a **separate**
pooled connection → 8× concurrent boot requests produce **zero** warnings.
The `(middleware)` frame seen in an early stack trace was a **webpack chunk
label, not an execution context**. The spun-off item
"[MÉDIA] pg cross-request warning in middleware" (from PR #106) is therefore
**misdiagnosed and closed** — no middleware fix is warranted.

**Fix status.** The *intra-handler* class (`Promise.all` of scoped ops
under `withTenant`) was resolved in PR #106 by sequencing 5 sites. The
remaining single-operation `include` concurrency has **no app-side fix** —
it is generated by the Prisma interpreter inside one operation, not by app
code. A durable fix would have to come **upstream** (a `@prisma/adapter-pg`
issue). Low priority; not now. Until then, treat the warning as noise.
